...
def get_args():
    ...
    parser.add_argument('--folder', default='normal', type=str, help='sub folder where the data files are')
...
def train(args):
    ...
    x_m = get_dataset(args.folder, args.speed, scaled=args.scaled, split=args.split_data, experiment_dir=args.experiment_dir)
    x = torch.tensor(x_m[:-1], dtype=torch.float)
    x_next = torch.tensor(x_m[1:], dtype=torch.float)

    autoencoder = MLPAutoencoder(args.input_dim, args.hidden_dim, args.latent_dim * 2, dropout_rate=args.dropout_rate_ae)
...
if __name__ == "__main__":
    ...
    path = "{}/saved_models/motor-{}-{}-{}-pairs{}{}{}.tar".format(args.save_dir, folder, args.speed,
                                                                     args.latent_dim, fixed, split, baseline)
